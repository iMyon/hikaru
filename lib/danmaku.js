...
class DanmakuStream extends EventEmitter{
    ...
    handleData(data) {
        const handleFrame = ({ magic, ver, action, param, payload }) => {
            ...
            if (ver === 2) return this._parseBinaryFrames(zlib.inflateSync(payload)).forEach(handleFrame)
...
class HighAvailabilityDanmakuStream extends EventEmitter {
    ...
    constructor(canonicalRoomId, opts = {}) {
        ...
        const {
            logPath = DANMAKU_LOG,
            redundancy = 1,    // additional servers to connect to
        } = opts

        this.autoRetry = true;

        this._redundancy = redundancy
        this._maxServerCount = 1 + this._redundancy
...
async function testHADanmaku(_roomid) {
    ...
    const dmk = new HighAvailabilityDanmakuStream(roomId, { redundancy: 2 })
    dmk.connect()
